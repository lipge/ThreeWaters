...
bool MysqlDao::addFriendApply(const int& fromUid, const int& toUid)
{
	...
	try {
		// 准备查询语句
		std::unique_ptr<sql::PreparedStatement> pstmt(con->_con->prepareStatement("INSERT IGNORE INTO friend_apply (from_uid, to_uid) VALUE (?,?)"));
...
bool MysqlDao::authFriendApply(const int& uid, const int& touid)
{
	...
	try {
		// 准备查询语句
		std::unique_ptr<sql::PreparedStatement> pstmt(con->_con->prepareStatement("CALL authFriendApplication(?,?)"));
		pstmt->setInt(1, uid);
		pstmt->setInt(2, touid);
		pstmt->execute();// 执行，当出现错误会被catch捕获
	}
	catch (sql::SQLException& e) {
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
...
{
	// 1.获取连接
	auto conn = _sqlpool->getConn();
	if (conn == nullptr) return nullptr;

	Defer defer([this, &conn] {
		_sqlpool->returnConn(std::move(conn));
		});
	std::shared_ptr<UserInfo> info = nullptr;	// 返回值
	
	try {
		// 2.准备sql语句
		std::unique_ptr<sql::PreparedStatement>	pstmt(conn->_con->prepareStatement("SELECT * FROM user WHERE `uid` = ?"));
		pstmt->setInt(1, uid);
		// 3.执行查询
		std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());
		// 4.遍历结果集
		if (res->next()) {
			info = std::make_shared<UserInfo>();	// 返回值
			info->uid = res->getInt("uid");
			info->pwd = res->getString("pwd");
...
std::shared_ptr<UserInfo> MysqlDao::getUser(std::string name)
{
	...
	try {
		// 2.准备sql语句
		std::unique_ptr<sql::PreparedStatement>	pstmt(conn->_con->prepareStatement("SELECT * FROM user WHERE `name` = ?"));
		pstmt->setString(1, name);
		// 3.执行查询
		std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());
		// 4.遍历结果集
...
bool MysqlDao::getFriendApplicationList(int uid, std::vector<std::shared_ptr<ApplyInfo>>& list)
{
	auto conn = _sqlpool->getConn();
	...
	try {
		...
		auto pstmt(conn->_con->prepareStatement(
			"SELECT u.* FROM `friend_apply` AS f JOIN `user` AS u "
			"ON f.from_uid = u.uid AND f.`status` = 0 "
			"WHERE f.to_uid = ?"));
		pstmt->setInt(1, uid);
...
bool MysqlDao::getFriendList(int uid, std::vector<std::shared_ptr<UserInfo>>& list)
{
	...
	try {
		...
		auto pstmt(conn->_con->prepareStatement(
			"SELECT u.* FROM `friend` AS f JOIN `user` AS u "
			"ON f.friend_id = u.uid "
			"WHERE f.self_id = ?"));
		pstmt->setInt(1, uid);
		auto res(pstmt->executeQuery());
		while (res->next()) {
