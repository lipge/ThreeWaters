...
struct AuthInfo {
    AuthInfo(int uid = -1
        , int sex = -1
        , QString name = "This field is not set"
        , QString nick = "This field is not set"
        , QString icon = "This field is not set"):
        _uid(uid), _sex(sex), _name(name), _nick(nick), _icon(icon){}
    int _uid;
    int _sex;
    QString _name;
    QString _nick;
...
//struct AuthRsp {
//    AuthRsp(int peer_uid = -1
//        , int peer_sex = -1
//        , QString peer_name = "∏√◊÷∂ŒŒ¥…Ë÷√"
//        , QString peer_nick = "∏√◊÷∂ŒŒ¥…Ë÷√"
//        , QString peer_icon = "∏√◊÷∂ŒŒ¥…Ë÷√") :
//        _uid(peer_uid), _sex(peer_sex), _name(peer_name), _nick(peer_nick), _icon(peer_icon) {
//    }
//    int _uid;
//    int _sex;
//    QString _name;
...
struct FriendInfo {
    ...
    FriendInfo(std::shared_ptr<AuthInfo> auth_info):_uid(auth_info->_uid),_name(auth_info->_name),
    _nick(auth_info->_nick),_icon(auth_info->_icon),
      _sex(auth_info->_sex){
        _last_msg = "Hello,im " + _name;
    }

    //FriendInfo(std::shared_ptr<AuthRsp> auth_rsp):_uid(auth_rsp->_uid),_name(auth_rsp->_name),
    //_nick(auth_rsp->_nick),_icon(auth_rsp->_icon),
    //  _sex(auth_rsp->_sex){}
